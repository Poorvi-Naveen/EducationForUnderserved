...
class _CourseRoadmapPageState extends State<CourseRoadmapPage>
  ...
  final String? _userId = FirebaseAuth.instance.currentUser?.uid;
  bool hasViewedCivicSense = false; // 🔥 Add this fla
  ...
  Future<void> _fetchChapters() async {
    ...
    try {
      ...
      int firstIncompleteIndex = fetchedChapters.indexWhere(
      ...
      setState(() {
        ...
        currentChapterIndex =
            firstIncompleteIndex != -1
                ...
                : fetchedChapters.length - 1; // All chapters completed
      });
  ...
  void startLesson() async {
    ...
    // ✅ Show civic sense popup only for 1st chapter
    if (currentChapterIndex == 0 && !hasViewedCivicSense) {
          ...
          MaterialPageRoute(
            builder:
                (context) => ExercisePage(
                  chapter: _chapters[currentChapterIndex],
                  course: widget.course,
                  onChapterCompleted: _markChapterCompleted,
                ),
          ),
        );
      }
    ...
    // Normal path (non-chapter 1 or civic sense already seen)
    Navigator.push(
      context,
      MaterialPageRoute(
        builder:
                    ...
                    onPressed: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder:
                              (context) =>
                                  ChapterInfoPage(course: widget.course),
                        ),
                      );
                    },
                  ),
