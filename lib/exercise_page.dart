...
class ExercisePageState extends State<ExercisePage> {
  ...
  Future<void> _fetchExercises() async {
    ...
    try {
      ...
      _checkChapterCompletion(); // check status after fetch
    } catch (e) {
  ...
  // if (_questions.every((q) => q["isCorrect"] == true)) {
  // setState(() {
  // _allQuestionsAnsweredCorrectly = true;
  // _exercisesCompleted = true;
  // widget.onChapterCompleted(widget.chapter.id);
  // });
  // }
  // }

  void _checkAnswer(String? selectedAnswer) {
    if (_isLoading || _questions.isEmpty || _currentUserId == null) return;
    ...
    setState(() {
      _feedbackMessage =
          isCorrect
              ? "Correct! You may now go to the next or previous question."
              : "Incorrect. Try again.";
      if (isCorrect) {
        _questions[_currentQuestionIndex]["isCorrect"] = true;
      }
    ...
    _updateIsCorrect(
      exerciseId,
      isCorrect,
    ).then((_) => _checkChapterCompletion());
  }

  Future<void> _checkChapterCompletion() async {
    try {
      final snapshot =
          await FirebaseFirestore.instance
              .collection('Course')
        ...
        child: Column(
          children: [
                  ...
                  for (var i = 1; i <= 4; i++)
                    Padding(
                      ...
                      child: ElevatedButton(
                        onPressed: () => _checkAnswer("Answer $i"),
                        child: Text("Answer $i"),
                      ),
                    ),
                ],
            ...
            Text(
              ...
              style: TextStyle(
                color:
                    _feedbackMessage.contains("Correct")
                        ? Colors.green
                        : Colors.red,
                fontWeight: FontWeight.bold,
              ),
            ...
            const SizedBox(height: 10),
            if (_allQuestionsAnsweredCorrectly)
              ElevatedButton(
